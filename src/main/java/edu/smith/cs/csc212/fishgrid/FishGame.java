	...
	public class FishGame {
		...
		public static final int Num_RC=5;
		
		public static final int Num_FF=4;
		...
		public FishGame(int w, int h) {
			...
			// Falling rock
				// Say that we want 3 falling rocks
				int NumFr=3;
				// For loop to call insert falling rocks
			for (int i=0; i<NumFr; i++) { 
				world.insertFallingRocks();
				}
		...
		public void step() {
			...
			for (WorldObject wo : overlap) {
				...
				if (missing.contains(wo)) {
					...
					// TODO
					//if ( ( ((Fish) wo).isPlayer()) && (((Fish) wo).getX()== Al.getX()){
						
				//	}
				
					//if fish is in same spot as food then collect earn points
				// TODO if ((((Fish) wo).isPlayer()) && ( ((Fish) wo) inSameSpot Snail) ) {}
				}
			}
		...
		public void click(int x, int y) {
			// (FishGrid) use this print to debug your World.canSwim changes!
